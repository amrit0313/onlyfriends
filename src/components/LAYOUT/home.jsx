const Home = () => {
  ...
  return (
    <div className=" flex">
      <div className="flex flex-col gap-10    w-3/4 h-screen bg-gradient-to-l from-indigo-200 overflow-scroll scrollbar-thumb-green-500 scrollbar-track-green-200">
        ...
        <div className="flex flex-col items-center mb-20">
          {data.map((items) => (
            <div
              key={items.id}
              className="flex flex-col  mx-5  px-10 py-20 bg-indigo-500/50 text-white rounded-xl mb-10 relative w-4/5 z-[1]"
            >
              ...
              >
                ~By
                <span className="text-yellow-200 ">{` ${items.username}`}</span>
              </p>
            </div>
          ))}
        </div>
      </div>
      <div className="flex flex-col items-start  text-white h-screen w-1/4 bg-slate-300">
        <p className="flex justify-center p-5 font-normal text-3xl bg-slate-900 w-full border-b-1 rounded-sm">
          <span> Filters</span>
        </p>
      </div>
    </div>
